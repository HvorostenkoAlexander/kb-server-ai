...
@Slf4j
@Service
public class TolThickMessageSender implements MessageSender, PdmMessageCreator {
    ...
    public TolThickMessageSender(@Value("${nsi.url.tol-thick}") String url_dictionary,
                                 @Value("${kafka.pdm.topic.tol-thick}") String topicName,
    ...
    public ResponseEntity<Long> send(PdmMessage message) {
        ...
        val sendingDto = pdmDtoConverter.toThicknessTkLimitDto(message.getDictionary());
    ...
    public PdmMessage createPdmMessage(ConsumerRecord record) {
        ...
        SpTolThick pdmObject = (SpTolThick) record.value();
