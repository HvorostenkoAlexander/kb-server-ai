...
@Service
public class Kc1ResultAdapter implements ApcsAvro, ResultAdapter<VerificationResultsKc1> {

    private static final Schema SCHEMA = VerificationResultsKc1.SCHEMA$;

    private final SimpleDateFormat dateFormatter;

    public Kc1ResultAdapter() {
        dateFormatter = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSXXX");
        dateFormatter.setTimeZone(TimeZone.getTimeZone("UTC"));
    }

    @Override
    public Class<VerificationResultsKc1> getSendingType() {
        return VerificationResultsKc1.class;
    }
    ...
    public RecordPk getPk(SpecificRecordBase recordBase) {
        try {
            return ((VerificationResultsKc1) recordBase).getPk();
        } catch (Exception e) {
    ...
    @Override
    public VerificationResultsKc1 adapt(ProductDto product, boolean isNew) {
        ...
        return VerificationResultsKc1.newBuilder()
                ...
                .setData(RecordKc1Data.newBuilder()
                        ...
                        .setMismatch(RecordKc1Mismatch.newBuilder()
                                ...
                                .build())
                        .setAttestationList(prepareRecordKc1AttLists(attestations))
                        .build())
                .build();
    }

    private List<RecordKc1AttList> prepareRecordKc1AttLists(List<AttestationDto> attestationList) {
        if (attestationList == null || attestationList.isEmpty()) {
            return List.of();
        }

        final var attestations = new ArrayList<RecordKc1AttList>();
    ...
    private List<RecordKc1AttListValues> prepareAttestationValue(List<AttestationDto> attResult, Group group) {
        return attResult.stream()
                ...
                .map(attestation -> {
                    ...
                    return RecordKc1AttListValues.newBuilder()
    ...
    private RecordKc1AttListNorms prepareNormLimit(AttestationDto attestation) {
        // в объекте AttestationDto ждем либо Equal, либо Min и (или) Max
        return RecordKc1AttListNorms.newBuilder()
                .setValueMin(attestation.getMin())
                .setValueMax(attestation.getMax())
                .setListAccValues(attestation.getEqual() == null ? null : List.of(
                        RecordKc1AttListNormsValues.newBuilder().setValue(attestation.getEqual()).build()
                ))
                .build();
    }

    private List<RecordKc1AttListParams> prepareParameters(AttestationDto attestation) {
        ...
        return map.entrySet().stream()
                .map(p -> RecordKc1AttListParams.newBuilder()
