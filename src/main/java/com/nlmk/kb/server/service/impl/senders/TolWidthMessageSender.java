...
@Slf4j
@Service
public class TolWidthMessageSender extends BaseSender implements MessageSender, PdmMessageCreator {
    ...
    public ResponseEntity<Long> send(PdmMessage message) {
        ...
        final var sendingDto = super.getPdmDtoConverter().toWidthTkLimitDto(message.getDictionary());
    ...
    public PdmMessage createPdmMessage(ConsumerRecord record) {
        SpTolWidth pdmObject = (SpTolWidth) record.value();
