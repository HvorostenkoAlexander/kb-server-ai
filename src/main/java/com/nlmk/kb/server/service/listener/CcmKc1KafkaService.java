...
public class CcmKc1KafkaService {
    ...
    public void receiveMessageReq(@Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
        ...
        log.info("receiveMessageReq (CCM KC1): topic [{}], partition [{}], offset [{}], key [{}], timestamp [{}], request.ts [{}], request.op [{}], request.pk.id [{}]",
        ...
        try {
            ...
            if (request.getOp() == EnumOp.D
                ...
                if (attResult.isEmpty() || Objects.isNull(attResult.get().getResult())) {
                    log.warn("receiveMessageReq (CCM KC1), нет результата аттестации для primeId {}, partition {}, offset {}, key {}",
