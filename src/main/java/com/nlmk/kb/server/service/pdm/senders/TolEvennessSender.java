...
@Service
public class TolEvennessSender extends BasePdmCreator {

    public TolEvennessSender(@Value("${kafka.pdm.topic.tol-evenness}") String type,
    ...
    Object getBody(PdmMessage message) {
        return super.getPdmDtoConverter().toEvennessTkLimitDto(message.getDictionary());
    }

    @Override
    PdmDictionary getDictionary(ConsumerRecord<Object, Object> consumerRecord) {
        final var pdmObject = (SpTolEvenness) consumerRecord.value();
