...
class SapMessageHandlerTest {
    ...
    void handleConsumerRecord() throws IOException {
        ...
        Mockito.when(s3Service.getObjectAsString(any(), any())).thenThrow(new S3ClientException("---"));
        ...
        Mockito.when(s3Service.getObjectAsString(any(), any())).thenReturn(s3zordersExample);
        Mockito.when(s3Service.getZorder(any())).thenThrow(new S3ClientException("---"));
        ...
        Mockito.verify(psmSender, never()).postZorder(any());
        ...
        Mockito.when(s3Service.getObjectAsString(any(), any())).thenReturn(s3zordersExample);
        Mockito.when(s3Service.getZorder(any())).thenReturn(parsedZorder);
        Mockito.when(psmSender.postZorder(any(ZORDERS051.class))).thenThrow(new HttpClientErrorException(HttpStatus.BAD_REQUEST));
        ...
        Mockito.verify(psmSender, times(1)).postZorder(any(ZORDERS051.class));

        // заказ передан в psm

        Mockito.reset(s3Service);
        Mockito.reset(psmSender);
        ...
        Mockito.verify(psmSender, times(1)).postZorder(captor5.capture());
